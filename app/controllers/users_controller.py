...
def login():
    ...
    user= users_model.get_by_email_password(email, password) #Va a traer el usuario si existe
    if not user:
        return RM.error("Upss, NO se encontro un usuario")
    if not EM.compare_hashes(user, user["password"]):
...
def get_user():
   ...
   if not user:
      return RM.error("Upss, Usuario no encontrado")
